...
class SeosambaWebmasters {
    ...
    const SEOSAMBA_PLATFORM_HOST      = 'mojo.seosamba.com';
    ...
    function mojo_website_id_card() {
        ...
        $website_id_card = array(
            'wicOrganizationName'        => filter_var( $this->_params['organization_name'], FILTER_SANITIZE_STRING ),
            'wicOrganizationDescription' => filter_var( $this->_params['organization_description'], FILTER_SANITIZE_STRING ),
            'wicPhone'                   => filter_var( $this->_params['phone'], FILTER_SANITIZE_STRING ),
            'wicCity'                    => filter_var( $this->_params['city'], FILTER_SANITIZE_STRING ),
            'wicCountryState'            => filter_var( $this->_params['state_name'], FILTER_SANITIZE_STRING ),
            'wicZip'                     => filter_var( $this->_params['zip'], FILTER_SANITIZE_STRING ),
            'wicIndustryType'            => filter_var( $this->_params['industry_name'], FILTER_SANITIZE_STRING )
        );

        foreach($website_id_card as $field => $value) {
            $query = "INSERT INTO " . $wpdb->prefix . "options (`option_name`, `option_value`, `autoload`)
                VALUES('" . $field . "','%s', 'no') 
                ON DUPLICATE KEY UPDATE option_value = '%s' ";
            $query = $wpdb->prepare( $query, $value, $value );
            $wpdb->query( $query );
        }
    ...
    public function gwt_code() {
        ...
        if ( !empty( $codes ) ) {
            foreach ( $codes as $code ) {
                echo '<meta name="google-site-verification" content="' . $code['value'] . '" >' . "\r\n";
            }
        }
    }
    ...
    private function _is_access_allowed() {
        ...
        return ($host === self::SEOSAMBA_PLATFORM_HOST && $access_key === $active_key);
    }
